...
// Total count
$cstmt = mysqli_prepare($conn, "SELECT COUNT(*) as c FROM orders o $where");
if ($types) mysqli_stmt_bind_param($cstmt, $types, ...$params);
mysqli_stmt_execute($cstmt);
$total = mysqli_fetch_assoc(mysqli_stmt_get_result($cstmt))['c'];
...
$active = 'orders';
$breadcrumb = ['Clients', 'Orders'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Orders – StoreHub</title>    <style>
    ...
    <div class="main-content">
        ...
        <div class="page-content">
            ...
            <div class="filter-bar">
                ...
                <a href="?status=processing" class="btn btn-sm <?= $statusFilter==='processing' ? 'btn-primary' : 'btn-secondary' ?>">Processing</a>
            ...
            <div class="table-card">
                <table class="data-table">
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td class="text-muted"><?= htmlspecialchars($o['client_email']) ?></td>
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:8px;white-space:nowrap">
                                    <a href="/storehub/manager/order_detail.php?id=<?= $o['id'] ?>"
                                       class="btn btn-secondary btn-sm">View</a>
                                    <select onchange="updateOrderStatus(<?= $o['id'] ?>, this.value)"
                                            class="filter-select" style="padding:5px 28px 5px 8px;font-size:12px">
                ...
                <div class="pagination">
                    <span>Showing <?= min($offset+1,$total) ?>–<?= min($offset+$perPage,$total) ?> of <?= $total ?></span>
...
<script>
// Quick order status update via AJAX
function updateOrderStatus(orderId, status) {
    if (!status) return;
    if (!confirm('Change order #' + orderId + ' to ' + status + '?')) return;

    fetch('/storehub/backend/orders/update_status.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'id=' + orderId + '&status=' + encodeURIComponent(status)
    })
