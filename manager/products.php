...
if ($isAjax) {
    if (mysqli_num_rows($products) === 0) {
        ...
        while ($p = mysqli_fetch_assoc($products)) {
            ...
            if ($p['stock']==0) $badge='<span class="badge badge-red">Out of Stock</span>';
            elseif($p['stock']<=$p['reorder_level']) $badge='<span class="badge badge-orange">Low Stock</span>';
            elseif($p['status']==='active') $badge='<span class="badge badge-green">● Active</span>';
            else $badge='<span class="badge badge-gray">Inactive</span>';
            ...
            echo '<tr>
                ...
                <td><div style="display:flex;gap:8px;white-space:nowrap">
                    ...
                    <a href="/storehub/backend/products/delete_product.php?id='.$p['id'].'" class="btn btn-danger btn-sm" onclick="return confirmDelete(\'Delete '.htmlspecialchars(addslashes($p['name'])).'?\')">Deactivate</a>
                </div></td>
            </tr>';
        }
    }
...
<head>
    ...
    <title>Products – StoreHub</title>    <style>
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:8px;white-space:nowrap">
                                    ...
                                    <a href="/storehub/backend/products/delete_product.php?id=<?= $p['id'] ?>"
                                       class="btn btn-danger btn-sm"
                                       onclick="return confirmDelete('Delete <?= htmlspecialchars(addslashes($p['name']), ENT_QUOTES) ?>?')">Deactivate</a>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
